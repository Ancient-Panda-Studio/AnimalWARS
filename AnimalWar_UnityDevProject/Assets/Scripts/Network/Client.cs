...
public class Client : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        ip = Constants.IpAdress;
        //ClientSend.SendLoginInfo(user, password);
        //ip = ReadIp(false);
        //port = int.Parse(ReadIp(true));
        if (instance == null)
        {
            instance = this;
        }
            ...
            while (_packetLength > 0 && _packetLength <= receivedData.UnreadLength())
            {
                ...
                ThreadManager.ExecuteOnMainThread(() =>
                {
                    using (var _packet = new Packet(_packetBytes))
                    {
                        ...
                        Debug.Log($"Packet Id Received was {_packetId} wich corresponds with {((ServerPackets) _packetId).ToString()}");
