...
{
    public static List<Map> ParseIntToMap(IEnumerable<int> mapOrder) { return mapOrder.Select(intMap => Dictionaries.Maps[intMap]).ToList(); }
    
    public static List<Map> ParseMapToInt(IEnumerable<int> mapOrder) { return mapOrder.Select(intMap => Dictionaries.Maps[intMap]).ToList(); }
